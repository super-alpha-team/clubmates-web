import ActivityItem, {
  ActivityProps as ActivityItemProps,
} from "components/kit/ActivityItem";
  ...
  return (
    <>
      <div className="mt-4">
        ...
        <span className="text-orange-dark italic">{searchKeyword}</span>
      </div>
      ...
      <div className="grid justify-items-center gap-y-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 m-8">
        {clubResults.map((result, index) => (
          <ClubItem key={index} data={result.data} />
        ))}
      </div>
    </>
  );
